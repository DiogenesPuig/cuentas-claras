...
- `aggregate.ts` / `aggregate.test.ts` — lógica pura de agregación:
  ...
  - `personaSpending`: gasto por persona — `key` (identidad de persona, para apodos MEJ-8), cuánto
    ...
    ninguna supera el 40%). Para "p1 = 50% (mayormente en super)". `categories` (MEJ-11): desglose
    completo del gasto de esa persona por categoría (monto + `share` relativo al gasto de la
    persona, no al total), de mayor a menor — no solo la dominante.
...
- `components/PersonaBreakdown.tsx` / `.test.tsx` — detalle del gasto por persona: % del total y
  categoría dominante ("mayormente en Super" / "varios"). Acompaña al donut de personas. Con la
  prop opcional `aliasing` (MEJ-8) muestra el apodo privado de cada persona y permite editarlo
  inline (✏️). MEJ-11: cada persona con categorías es expandible (▸/▾) y muestra el desglose
  completo (`person.categories`) con monto y % del gasto de esa persona por categoría.
