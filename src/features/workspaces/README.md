...
- `api.ts` — Supabase: `listMyWorkspaces`, `createWorkspace`, `getMyRole`, `getWorkspace`,
  ...
  (MEJ-15: borra los **archivos** de comprobantes del bucket —la cascada de la DB no los toca— y luego
  el grupo con todo lo colgado por cascada; RLS `ws_delete` exige **owner**), `updateMemberRole`,
